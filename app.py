        ...
        for loop in range(max_loops):
            ...
            if response.status_code == 200:
                ...
                if result.get('code') == 200:
                    ...
                    if not next_token or next_token == "" or next_token == "None":
                ...
                if rows:
                    ...
                    if account_info:
                        ...
                        if platform == "🎵 TikTok":
                            ...
                            c2.metric("粉丝数", f"{account_info.get('followers', 0):,}")
                            c3.metric("总获赞", f"{account_info.get('total_likes', 0):,}")
                        elif platform == "📺 YouTube":
                            c1, c2, c3 = st.columns(3)
                            c1.metric("频道", account_info.get('username', ''))
                            c2.metric("订阅数", f"{account_info.get('subscribers', 0):,}")
                            c3.metric("总视频数", f"{account_info.get('total_videos', 0):,}")
                    ...
                    c1.metric("总播放量", f"{safe_sum(df['播放量']):,}")
                    c2.metric("总点赞数", f"{safe_sum(df['点赞数']):,}")
